...
//        Imgproc.threshold(temp, temp, minVal, maxVal, Imgproc.THRESH_BINARY);
        ...
        for (int i=0; i<img.size(); i++){
            //细化算法
            dst = Thinning.ImgThin(img.get(i));
            ledNum.add(Thinning.HoughPrecess(dst));
        }
...
//            Imgproc.circle(dst, end, 2, new Scalar(0, 255, 0));
        ...
        List<Point> pointStart = Thinning.pickUp_line(starts);
        List<Point> pointEnd = Thinning.pickUp_line(ends);
